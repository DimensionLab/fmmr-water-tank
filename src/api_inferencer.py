...
from modulus.utils.io.vtk import var_to_polyvtk, VTKBase, VTKUniformGrid
from modulus.dataset import DictInferencePointwiseDataset


class PointwiseAPIInferencer(Inferencer):
    """
    Pointwise Inferencer that allows inferencing on pointwise data by requests to API endpoint
    ...
    def export_onnx(self, name, results_dir):
        for i, (invar0,) in enumerate(self.dataloader):
            # Move data to device
            invar = Constraint._set_device(
                invar0, device=self.device, requires_grad=self.requires_grad
            )
            torch.onnx.export(self.model, (invar, {}), f"{results_dir}/{name}.onnx.pb", export_params=True)
            import pickle
            with open(f"{results_dir}/{name}_data.pkl", 'wb') as f: pickle.dump(invar, f)
            break
